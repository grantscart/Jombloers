...
module.exports = {
  ...
  execute: async (message) => {
    ...
    if (command) {
        if(command.ownerOnly && message.author.id !== config.owner) return message.reply({content: "owner only ya."});
      if (command.cooldown) {
        if (cooldown.has(`${command.name}-${message.author.id}`)) {
        const nowDate = message.createdTimestamp;
        const waitedDate = cooldown.get(`${command.name}-${message.author.id}`) - nowDate;
          return message.reply({
            content: `Lagi Cooldown <t:${Math.floor(new Date(nowDate + waitedDate).getTime() / 1000)}:R> bentar ya.`,
