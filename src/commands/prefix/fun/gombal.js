...
module.exports = {
    ...
    ownerOnly: false,//komutu sadece geliştiricinin kullanabilmesini istersen true olarak değiştir
     run: async (client, message, args, prefix) => {
        // execute
        if (message.mentions.users.size < 1) return await message.reply("Mention orang yang mau kamu gombalin dong")
          .catch(console.error);
        if (message.mentions.users.size > 1) return await message.reply("Banyak amat yang mau kamu gombalin, tag satu aja")
          .catch(console.error);
      var listGombalan = [
...
"I don't know about my future, but when I look into your eyes, then I know my future is you."
      ...
      await message.channel.send(`${message.mentions.users.first().toString()}, ada gombalan nih dari ${message.author.tag}\n\n${listGombalan[Math.floor(Math.random() * listGombalan.length)]}`)
        .catch(console.error);
    },
};